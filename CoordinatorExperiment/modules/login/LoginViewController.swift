...
class LoginViewController: UIViewController {
  ...
  lazy var customView: LoginView = {
    let customView = LoginView(frame: CGRect(x: 0, y: 0, width: 320, height: 480))
    return customView
  }()
  ...
  private func configureRx() {
    guard let model = viewModel else {
      assertionFailure("Please, set ViewModel as dependency for Login")
      return
    }

    let input = LoginViewModel.Input(appearState: viewAppearState, closeSelf: customView.button.rx.tap.asObservable())
  ...
  deinit {
    print("LoginViewController dead")
  }
}
