...
class SignInViewController: UIViewController {
  ...
  // IBOutlet & UI
  lazy var customView: SignInView = {
    let customView = SignInView(frame: CGRect(x: 0, y: 0, width: 320, height: 480))
    return customView
  }()
  ...
  private func configureRx() {
    guard let model = viewModel else {
      assertionFailure("Please, set ViewModel as dependency for SignIn")
      return
    }

    let input = SignInViewModel.Input(
      appearState: viewAppearState,
      tryLogin: customView.enterButton.rx.tap.asDriver()
    )
  ...
  deinit {
    print("SignInViewController dead")
  }
}
