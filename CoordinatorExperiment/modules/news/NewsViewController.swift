...
class NewsViewController: UIViewController {
  ...
  // IBOutlet & UI
  lazy var customView: NewsView = {
    let customView = NewsView(frame: CGRect(x: 0, y: 0, width: 320, height: 480))
    return customView
  }()
  ...
  private func configureRx() {
    guard let model = viewModel else {
      assertionFailure("Please, set ViewModel as dependency for News")
      return
    }

    let input = NewsViewModel.Input(appearState: viewAppearState)
